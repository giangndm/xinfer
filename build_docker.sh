...
# Build xInfer Docker image with automatic GPU/CUDA detection.
#
# Flags:
...
#   3: IMAGE_TAG     (default: xinfer:latest)
#   4: CHINA_MIRROR  Auto-detected (China network → 1), or 0/1 to override
#
# Features auto-selected from SM:
#   SM80+      → cuda,nccl,python,flashinfer,cutlass
#   SM70/SM75  → cuda,nccl,python (no flashinfer/cutlass)
#
# Override via environment variables:
#   WITH_FEATURES="cuda,nccl,python" ./build_docker.sh

usage() {
  cat <<'EOF'
Usage:
  ./build_docker.sh [--prod|-p] [SM_ARG] [CUDA_VERSION] [IMAGE_TAG] [CHINA_MIRROR]
...
Auto-detection rules:
  SM70/SM75  → CUDA 12.9.0, features: cuda,nccl,python
  SM80+      → CUDA 13.0.0, features: cuda,nccl,python,flashinfer,cutlass
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    --prod|-p) DOCKERFILE="Dockerfile.prod"; shift ;;
    --help|-h) usage; exit 0 ;;
    --)
      shift
...
if [[ "$SM_NUM" -lt 80 ]]; then
  DEFAULT_CUDA="12.9.0"
  DEFAULT_FEATURES="cuda,nccl,python"
else
  DEFAULT_CUDA="13.0.0"
  DEFAULT_FEATURES="cuda,nccl,python,flashinfer,cutlass"
fi
